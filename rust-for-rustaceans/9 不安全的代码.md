仅仅提及不安全代码常常会引起 Rust 社区中许多人以及许多旁观者的强烈反应。 虽然有些人认为这“没什么大不了的”，但另一些人则谴责这是“Rust 的所有承诺都是谎言的原因”。 在本章中，我希望稍微拉开帷幕，解释什么是不安全，什么不安全，以及应该如何安全地使用它。 在撰写本文时，也可能在您阅读本文时，Rust 对不安全代码的精确要求仍在确定中，即使它们全部确定下来，完整的描述也将超出本书的范围。 相反，我会尽力为您提供构建块、直觉和工具，帮助您浏览最不安全的代码。

本章的主要收获应该是：不安全代码是 Rust 为开发人员提供的利用不变量的机制，无论出于何种原因，编译器都无法检查这些不变量。 我们将看看 unsafe 是如何做到这一点的，这些不变量可能是什么，以及我们可以用它做什么。

> **不变量** 
> 
> 在本章中，我将大量讨论不变量。 不变性只是一种奇特的表达方式，表示“为了让你的程序正确，某些东西必须是真实的”。 例如，在 Rust 中，一个不变量是使用 & 和 &mut 的引用不会悬空——它们总是指向有效值。 您还可以具有特定于应用程序或库的不变量，例如“头指针始终位于尾指针之前”或“容量始终是 2 的幂”。 最终，不变量代表了代码正确所需的所有假设。 然而，您可能并不总是了解代码使用的所有不变量，这就是错误蔓延的地方。

至关重要的是，不安全代码并不是一种规避 Rust 各种规则（例如借用检查）的方法，而是一种使用编译器之外的推理来强制执行这些规则的方法。 当您编写不安全的代码时，您有责任确保生成的代码是安全的。 在某种程度上，当 unsafe 作为关键字用于通过 unsafe {} 允许不安全操作时，它会产生误导； 并不是说所包含的代码*不*不安全，而是代码被允许执行其他不安全的操作，因为在这个特定的上下文中，这些操作*是*安全的。

本章的其余部分分为四个部分。 我们将首先简要检查关键字本身的使用方式，然后探讨 unsafe 允许您执行哪些操作。 接下来，我们将了解编写安全的不安全代码必须遵循的规则。 最后，我会给你一些关于如何真正安全地编写不安全代码的建议。
## unsafe关键字

在我们讨论不安全赋予您的权力之前，我们需要先讨论一下它的两种不同含义。 unsafe 关键字在 Rust 中具有双重用途：它将特定函数标记为调用不安全*并且*它使您能够在特定代码块中调用不安全功能。 例如，清单 9-1 中的方法被标记为不安全，即使它不包含不安全代码。 在这里，unsafe 关键字向调用者发出警告，即编写调用 decr 的代码的人必须手动检查额外的保证。
...
这两个清单在 unsafe 的使用上有所不同，因为它们体现了不同的合同。 decr 要求调用者在调用该方法时要小心，而 as_ref 假设调用者在调用其他不安全方法（如 decr）时*非常*小心。 要了解原因，请想象 SomeType 实际上是一个引用计数类型，如 Rc。 尽管 decr 只递减一个数字，但该递减可能会通过安全方法 as_ref 依次触发未定义的行为。 如果您调用 decr，然后删除给定 T 的倒数第二个 Rc，则引用计数将降至零，并且 T 将被删除 - 但程序仍可能对最后一个 Rc 调用 as_ref，并最终得到悬空 参考。

**N O T E** *未定义的行为描述了程序在运行时违反语言不变量的后果。 一般来说，如果程序触发未定义的行为，结果是完全不可预测的。 我们将在本章后面更详细地介绍未定义的行为。* 

相反，只要无法使用安全代码破坏 Rc 引用计数，就可以像 as_ref 的代码那样取消引用 Rc 内的指针 — &self 存在的事实证明该指针仍然必须 是有效的。 我们可以使用它为调用者提供一个安全的 API 来执行不安全的操作，这是如何负责任地使用不安全的核心部分。

由于历史原因，如今 Rust 中每个不安全的 fn 都包含一个隐式的不安全块。 也就是说，如果声明一个不安全的 fn，则始终可以调用该 fn 内的任何不安全方法或原始操作。 然而，该决定现在被认为是一个错误，目前正在通过已接受和实施的 RFC 2585 进行恢复。该 RFC 警告存在一个不安全的 fn，该 fn 会在内部没有显式不安全块的情况下执行不安全的操作。 在 Rust 的未来版本中，lint 也可能成为一个严重错误。 这个想法是为了减少“脚枪半径”——如果每个不安全的 fn 都是一个巨大的不安全块，那么你可能会在没有意识到的情况下意外地执行不安全的操作！ 例如，在清单 9-1 的 decr 中，根据当前规则，您还可以添加 *std::ptr::null() 而不带任何不安全注释。

作为标记的不安全和作为启用不安全操作的机制的不安全块之间的区别很重要，因为您必须以不同的方式考虑它们。 不安全的 fn 向调用者表明，他们在调用有问题的 fn 时必须小心，并且必须确保函数记录的安全不变量成立。

同时，不安全的块意味着编写该块的人仔细检查其内部执行的任何不安全操作的安全不变量是否成立。 如果你想要一个近似的现实世界的类比，不安全的 fn 是一个未签署的合同，要求调用代码的作者“庄严宣誓 X、Y 和 Z”。 同时，unsafe {} 是调用代码的作者签署了该块中包含的所有不安全合约。 当我们阅读本章的其余部分时，请记住这一点。

## Great Power

那么，一旦你与 unsafe {} 签署了不安全合约，你可以做什么？ 老实说，没那么多。 或者更确切地说，它没有启用那么多新功能。 在不安全的块内，您可以取消引用原始指针并调用不安全的 fns。

就是这样。 从技术上讲，您还可以执行其他一些操作，例如访问可变和外部静态变量以及访问联合字段，但这些不会对讨论产生太大影响。 老实说，这就足够了。 这些功能一起使您能够造成各种破坏，例如使用 mem::transmute 将类型转换为另一种类型、取消引用指向谁知道在哪里的原始指针、将 &'a 转换为 &'static，或者使类型可跨线程共享 边界，即使它们不是线程安全的。

在本节中，我们不会太担心这些权力可能会出现什么问题。 我们将把它留给接下来的无聊、负责任、成熟的部分。 相反，我们将看看这些整洁闪亮的新玩具以及我们可以用它们做什么。
...
由于原始指针没有生命周期，因此可以在 Rust 生命周期系统中无法静态表达所指向值的活跃度的情况下使用它们，例如自引用结构中的自指针，如我们在 第 8 章。只要 self 存在（并且不移动，这就是 Pin 的用途），指向 self 的指针就有效，但这不是您通常可以命名的生命周期。 虽然整个自引用类型可能是“静态的”，但自指针却不是——如果它是静态的，那么即使您将该指针交给其他人，他们也可以永远继续使用它，即使在 self 被使用之后也是如此。 走了！ 以清单9-3中的类型为例； 在这里，我们尝试将构成值的原始字节与其存储的表示一起存储。
