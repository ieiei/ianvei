...
这次我分享的题目是数元灵开源一年多的数据湖产品Lakesoul
另外我们数元灵还开源了另外一个产品，一站式机器学习平台MetaSpore 
在上面可以做一很多这种搜索
他的这个业务大家带来非常方便的一个体验

Lakesoul在我们公司主要的定位是我们希望给他构建成一个端到端的开源实时湖仓的框架
围绕这个框架呢
然后框架呢就是说
...
所以在正式讲解技术主题之前，我先介绍一下湖仓一体的技术背景

说这个背景主要是说还是离不开云原生
云原生出来之后给我们带来很多这种方便快捷的体验，
比如随便拉一个镜像，搭几台集群，基本上通过一件事或几行命令就能把一个集群或者一个应用部署上去，这极大减少了我们的使用和维护成本。
但是说云原生在底层
其实都是以对象存储为基础的
对象存储带来一个最大的好处就是特别廉价，并且它的性能也不差，存储的格式又多，比如结构化、非结构化、还有半结构化的数据等等。

这样的一个廉价的存储，能给我们数据分析带来什么样的好处呢？
其实就是说能存储的数据越来越多哦
就是说这种大力出奇迹的这种东西
就会带来很多意想不到的收获。

首先是带来很多模式的转变，比如说现在就是说大家都后来就是说
呃对象存储之后，有一段时间特别兴起的一句话：”数据入湖“
而且出现了一个新的工种： “入湖工程师”
然后这有这么一个工种
像那时候就是说
为什么就是数据首先入湖
...
就是说涉及到一个使用方数据的使用
处理方式的一个转变
我们现在归结为他现在处理方式是从传统的etl转成到现在的elt
elt的含义可以理解为：
- e: 是从源头拉数据过来
- l: 是先加载在我我这个数据湖仓里面，就是先加载数据然后再做处理
- t: transform，就是说再做处理，比如做一个count，或者做一个这么自断抽取，或者做一个什么处理
哦这样的一种模式
那传统大家就问传统的模式是怎样呢
传统模式就是咱说的etl
etl就是说我新数据抽取还不变哦
...
如果还需要日志型的数据话，就需要存在Elasticsearch引擎里面，这里面会造成了一些困扰：
...
2. 存储不一致：比如离线数据存储T+1的数据，日志数据存入elasticsearch，实时数据存入clickhouse。这会导致整体成本都很高：
	...
	b. 开发成本也高：例如数据分析师对某个表提出了一个新的处理方式，比如新增了几列或者修改了几列，需要及时同步数据工程师对相应的任务进行调整，对任务进行不断的修正。在传统模式下，先把T任务按照数据分析师的要求重新转换，再把T+1的数据重新从源端拉取回来。但是源端的数据可能存在保质期（比如7天或者一个月），如果需要拉取更久的时间，数据可能就存在丢失，数据的价值也就体现不出来。
但是如果你现在如果你
就是说这是另外一个
但是还有一个就是说你对上层
上层统一的话
...
所以结合我们的背景总结，其实我们对lakesoul的一个定位
...
数据是这么数据是数据内容是这样的
就是有一个base文件
这base文件有1,000万数据1,000万工具总共11个文件
base文件是啊1,000万
然后每次每次会有一个增量数据
这个增量数据是有10个
这10个是有版本的
...
非主键的那些列自动给它扩充
就最后就表现成一个大宽表的形式
大家可以去试一下这个几倍几十倍的提高都是没有问题的
而且这个上面就说怎么用
或者说大家可以去
关注我们这个使用教程
这里面就是嗯
大家如果还是不理解的话，可以在我们的微信群里面都可以支和一声
另外就是说在机器学习里面
其实说做这个比如说很很简单的这种
个性化推进啊还有营销行动
都会有一个大家比较认知的说
...
而现在我们要把它做成与引擎无绑定的compaction
另外就是说我们实现一个native merge into的能力
其实我们以前也有merge into这样一吐的
这个语句的能力
但是只是针对主键表
后面我会做非主键表
主键表就要做一个merge into的这个
表达能力
另外就是说在生态上面我们
还要扩大扩大
就是说我们现在
...
肯定走我们这种比如说Python的接口
或者走c的接口这种方式
另外就是说我们还差了一些数据湖的一个东西
就是日志
我们还要考虑进来是日志
我们的数据湖和像以前一样
就是说我们
让用户尽可能让用户配置简单
你可能简单的配置一下就可以不用关心中间的一些细节
嗯另外就是说我们还有要提供一个服务层
...
是我们后面着重要要弄的一个事啊
另外就是还有一个是最近
我们现在正在捐献给这个linux foundation AI&data这个开源组织
我们想要通过这种引入中立社区
这种运作的方式
来扩大我们的这个lakesoul的影响力
也扩大国产湖仓的这个影响力啊
